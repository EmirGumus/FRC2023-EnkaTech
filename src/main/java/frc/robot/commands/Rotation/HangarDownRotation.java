...
public class HangarDownRotation extends CommandBase {
  private final RotationSub m_RotationSub;
  private double m_speed;
  private double r_encoder;
  double error = -15;

  public HangarDownRotation(RotationSub _RotationSub) {
  ...
  public void execute() {
    ...
    m_speed = (Constants.torso.m_distance_rotationHangarDown+error-r_encoder)* Constants.torso.gripperKP;
    m_RotationSub.runRotation(m_speed);
 
    if(r_encoder <= Constants.torso.m_distance_rotationHangarDown){
